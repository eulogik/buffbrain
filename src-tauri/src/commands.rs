...
pub async fn set_config(
    ...
    if let Some(ref tray) = *state.tray.lock().unwrap() {
        if let Some(st) = show_tray {
            state
                .db
                .set_config("show_tray", if st { "true" } else { "false" })
                .map_err(|e| e.to_string())?;
            tray.set_visible(st).map_err(|e| e.to_string())?;
        }
    }
    ...
    if let Some(aut) = auto_start {
        state
            .db
            .set_config("auto_start", if aut { "true" } else { "false" })
            .map_err(|e| e.to_string())?;
